    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if e.code in (429, 500, 502, 503) and attempt < MAX_RETRIES - 1:
                ...
                print(f"  Retrying after HTTP {e.code} (attempt {attempt + 1}/{MAX_RETRIES}, waiting {wait}s)...", file=sys.stderr)
                time.sleep(wait)
                continue
            raise
            ...
            if attempt < MAX_RETRIES - 1:
                wait = RETRY_BACKOFF ** (attempt + 1)
                print(f"  Retrying after network error (attempt {attempt + 1}/{MAX_RETRIES}, waiting {wait}s): {e}", file=sys.stderr)
                time.sleep(wait)
                continue
            raise
...
def fetch_latest_event(issue_id: str) -> dict:
    try:
        ...
        print(f"    WARNING: failed to fetch event data for issue {issue_id} "
...
def load_skip_ids() -> dict:
    """Load the skip file as {glitchtip_issue_id: jira_ticket_key}.

    Ticket keys are used to comment on existing tickets when an already-known
    error pattern recurs. Older skip files stored a plain list of issue IDs;
    those are loaded with an empty (unknown) ticket key.
    """
...
def save_skip_ids(skip_ids: dict) -> None:
    skip_file = os.environ.get("GLITCHTIP_SKIP_FILE", ".glitchtip-skip-ids.json")
    with open(skip_file, "w") as f:
        json.dump(dict(sorted(skip_ids.items())), f)
...
def create_jira_ticket(issue: dict, event: dict, project_name: str, group: dict = None) -> str:
    ...
    if text:
        try:
            ...
            raise RuntimeError(f"Unexpected Jira MCP response: {text[:500]}", file=sys.stderr)
        return ticket.get("key", "unknown")
    return "unknown"


def add_recurrence_comment(ticket_key: str, new_issue_ids: list, project_name: str) -> None:
    """Comment on an existing ticket when its error pattern recurs with new GlitchTip issue IDs."""
    links = ", ".join(f"[#{iid}|{glitchtip_issue_url(iid)}]" for iid in new_issue_ids)
    body = (
        f"New occurrence(s) of this error pattern detected in GlitchTip ({project_name}): {links}."
    )
    call_jira_mcp("jira_add_comment", {"issue_key": ticket_key, "comment": body})
...
def main():
    ...
    for slug, project_name in slug_map.items():
        ...
        # Group by normalized title BEFORE filtering out already-ticketed issues,
        # so a new occurrence of a known error pattern still lands in the same
        # group as its already-ticketed siblings (partial overlap detection).
        ...
        for group in groups:
            ...
            all_group_issues = group["issues"]
            new_issues = [i for i in all_group_issues if str(i.get("id", "")) not in skip_ids]
            known_issues = [i for i in all_group_issues if str(i.get("id", "")) in skip_ids]
            title = group["representative"].get("title", "unknown")

            if not new_issues:
                # Every issue in this pattern already has a Jira ticket.
                total_skipped += len(all_group_issues)
                continue

            if known_issues:
                # Recurrence: this error pattern already has a ticket, but new
                # GlitchTip issues matching it have shown up. Comment instead
                # of creating a duplicate ticket.
                new_ids = [str(i.get("id", "")) for i in new_issues]
                ticket_key = next(
                    (skip_ids[str(i.get("id", ""))] for i in known_issues
                     if skip_ids.get(str(i.get("id", "")))),
                    "",
                )
                if DRY_RUN:
                    print(f"  [DRY RUN] Recurrence of known pattern, {len(new_ids)} new "
                          f"occurrence(s), would comment on existing ticket: {title}")
                elif ticket_key:
                    try:
                        add_recurrence_comment(ticket_key, new_ids, project_name)
                        print(f"  Commented on {ticket_key} for {len(new_ids)} new "
                              f"occurrence(s): {title}")
                    except Exception as e:
                        print(f"  FAILED to comment on {ticket_key}: {e}", file=sys.stderr)
                    for iid in new_ids:
                        skip_ids[iid] = ticket_key
                else:
                    print(f"  Recurrence of known pattern but no tracked ticket key "
                          f"(pre-existing skip entry), not commenting: {title}")
                total_skipped += len(new_issues)
                continue

            # Brand-new error pattern.
            issue = group["representative"]
            issue_id = str(issue.get("id", ""))
            group_size = len(new_issues)
            ...
            if DRY_RUN:
                ...
                try:
                    ...
                    created.append({"issue_id": issue_id, "ticket": ticket_key, "title": title,
                                    "group_size": group_size, "project": project_name})
                    for i in new_issues:
                        skip_ids[str(i.get("id", ""))] = ticket_key
                except Exception as e:
                    print(f"    FAILED to create ticket: {e}", file=sys.stderr)
                    failed.append({"issue_id": issue_id, "title": title, "error": str(e),
                                   "project": project_name})

    # Persist skip IDs (with ticket keys) so re-runs don't create duplicates and
    # recurrences of known patterns can be commented on the right ticket.
    if not DRY_RUN:
        save_skip_ids(skip_ids)
